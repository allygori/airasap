...
export function SummaryAccordion({
  ...
  return (
    <Accordion
      ...
      className="w-full rounded-md border bg-white dark:bg-gray-950"
    >
      {data.map((group, index) =>
        group.subItems && group.subItems.length > 0 ? (
          ...
          >
            <AccordionTrigger className="py-4 hover:no-underline">
              <div className="flex w-full justify-between pr-4">
                ...
                <span
                  className={`font-semibold ${group.value < 0 ? 'text-destructive' : ''}`}
                >
          ...
          <div
            key={`${group.name}-${index}`}
            className="flex justify-between border-b px-4 py-4 last:border-0"
          >
