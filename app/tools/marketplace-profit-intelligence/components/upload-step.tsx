...
type UploadStepProps = {
  ...
  onIncomeFileChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  onOrderFileChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  onUpload: () => void;
};
    ...
    <div className="space-y-12">
      <Card className="mx-auto max-w-2xl border-2 border-dashed">
        <CardHeader className="pb-2 text-center">
          ...
          <CardDescription>Pilih berkas Excel pelepasan dana dan pesanan selesai (.xlsx)</CardDescription>
        </CardHeader>
        <CardContent className="mx-auto flex w-full max-w-md flex-col gap-6 pb-8">
          <div className="flex flex-col gap-2">
            ...
            <input type="file" accept=".xlsx" className="hidden" ref={incomeInputRef} onChange={onIncomeFileChange} />
            <Button
              variant="outline"
            ...
            >
              ...
              <span className="truncate">{incomeFile ? incomeFile.name : "Pilih Berkas Pelepasan Dana"}</span>
            </Button>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="file" accept=".xlsx" className="hidden" ref={orderInputRef} onChange={onOrderFileChange} />
            <Button
              variant="outline"
              className="h-11 justify-start truncate border-dashed px-4"
            ...
            >
              <FileSpreadsheet className="mr-2 h-4 w-4 flex-shrink-0 text-green-600" />
              <span className="truncate">{orderFile ? orderFile.name : "Pilih Berkas Pesanan Selesai"}</span>
            </Button>
          </div>

          {incomeFile && orderFile && (
            <Button onClick={onUpload} disabled={isUploading} className="mt-4 h-11 w-full">
              {isUploading ? "Menganalisis Berkas Gabungan..." : "Analisis Sekarang"}
            </Button>
          )}
        </CardContent>
      </Card>

      <div className="mx-auto max-w-3xl space-y-4 opacity-50 pointer-events-none select-none">
