...
export default function MarketplaceProfitIntelligenceUploadPage() {
  ...
  const handleIncomeFileChange = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch("/api/profit-intelligence/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.success && data.sid) {
        router.push(`/tools/marketplace-profit-intelligence/${data.sid}`);
        return;
      }

      setErrorMessage(data.error || "Gagal menganalisis file. Silakan cek format file lalu coba lagi.");
    } catch {
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="space-y-2 text-center">
          ...
          <p className="text-muted-foreground">
            Upload Income Sudah Dilepas dan Download Pesanan Selesai dari Shopee untuk mulai analisis.
          </p>
        </div>
