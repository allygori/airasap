...
function ActionSheetTrigger({
  ...props
}: ActionSheetPrimitive.Trigger.Props) {
  return (
    <ActionSheetPrimitive.Trigger
...
function ActionSheetPortal({
  ...props
}: ActionSheetPrimitive.Portal.Props) {
  return (
    <ActionSheetPrimitive.Portal
      data-slot="action-sheet-portal"
      {...props}
    />
  );
}
...
function ActionSheetOverlay({
  className,
  ...props
}: ActionSheetPrimitive.Backdrop.Props) {
  return (
  ...
  return (
    <ActionSheetPortal>
      ...
      <ActionSheetPrimitive.Popup
        ...
        className={cn(
          'bg-background text-foreground ring-border/50 fixed inset-x-0 bottom-0 z-50 mx-auto mt-24 flex w-full max-w-lg flex-col gap-4 rounded-t-xl p-6 text-sm shadow-xl ring-1 transition duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] outline-none data-ending-style:translate-y-full data-starting-style:translate-y-full',
          className
        )}
        {...props}
      >
        <div className="bg-muted absolute top-3 left-1/2 h-1.5 w-12 -translate-x-1/2 rounded-full" />
        {children}
      </ActionSheetPrimitive.Popup>
    </ActionSheetPortal>
  );
...
function ActionSheetHeader({
  ...
  return (
    <div
      ...
      className={cn(
        'mt-2 flex flex-col gap-1.5 text-center sm:text-left',
        className
      )}
      {...props}
    />
...
function ActionSheetTitle({
  className,
  ...props
}: ActionSheetPrimitive.Title.Props) {
  return (
...
function ActionSheetConfirm({
  ...
  const { onConfirm } = React.useContext(
    ActionSheetContext
  );
